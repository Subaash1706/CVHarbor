...
const MitTemplate3 = React.forwardRef((props, ref) => {
                    ...
                    <div>
                        ...
                        <div>{
                            obj[item].map((item, ind)=>{
                                return(
                                    <div key={ind}>
                                        <TitleDate title={item.name} date={item.date?`${item.date.split('-')}`:''}/>
                ...
                <Hr />
                {
                    education.map((item, ind)=>{
                        ...
                        <div key={ind}>
                            ...
                            <FlexBox style={{justifyContent: 'space-between'}} direction='row'>
                                <div>{item.course}, {item.stream}</div>
                                <div>{item.end_date ? `${item.end_date.split('-')[1]} ${item.end_date.split('-')[0]}` : item.end_date}</div>
                            </FlexBox>
                            {item.accomplishments &&<OutwardList listItems={item.accomplishments}/>}
                        </div>
